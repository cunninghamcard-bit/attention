/**
 * Input: ../app/App, ../app/SettingRegistry, ../ui/Setting, ../ui/Icon
 * Output: CorePluginsSettingTab
...
export class CorePluginsSettingTab implements SettingTab {
  ...
  display(): void {
    ...
    for (const state of states) {
      const haystack =
        `${state.definition.name} ${state.definition.description ?? ""} ${state.id}`.toLowerCase();
      ...
      const setting = new Setting(group.itemsEl)
        ...
        .addToggle((toggle) =>
          toggle.setValue(state.enabled).onChange((enabled) => {
            const action = enabled
              ? this.app.internalPlugins.enable(state.id, true)
              : this.app.internalPlugins.disable(state.id, true);
      ...
      if (state.enabled && pluginSettingsTab) {
        setting.addButton((button) =>
          button.setButtonText("Options").onClick(() => {
            this.app.setting.openTabById(state.id);
          }),
        );
      }
      const hasCommands = this.app.commands
        .getCommands()
        .some((command) => command.id.startsWith(`${state.id}:`));
      if (state.enabled && hasCommands && this.app.setting.getTabById("hotkeys")) {
        setting.addButton((button) =>
          button.setButtonText("Hotkeys").onClick(() => {
            const hotkeysTab = this.app.setting.getTabById("hotkeys") as {
