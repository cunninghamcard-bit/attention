...
export class GitNavView extends ItemView {
  ...
  private buildShell(): void {
    ...
    //
    // It cannot ride the leaf's view-header instead: app.css hides that
    // outright in both side splits, so an addAction there renders nothing.
    const headerEl = createDiv("nav-header", root);
    const buttonsEl = createDiv("nav-buttons-container git-nav-modes", headerEl);
    this.modeButtonEls = {
