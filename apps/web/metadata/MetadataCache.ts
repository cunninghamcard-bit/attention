/**
 * Input: ../core/Events, ./FrontmatterTags, ./MetadataCacheStore, ../core/PropertyValue, ../vault/TAbstractFile, ../vault/Vault, ./BlockCache, ./MarkdownMetadataParser
...
import { MarkdownBlockCache } from "./BlockCache";
...
export class MetadataCache extends Events {
  ...
  private metadataWorker: Worker | null | undefined;
  ...
  constructor(
    ...
    // The unique-name lookup follows the vault from construction: bucket
    // upkeep is cheap per event, and link-path resolution must stay correct
    // on a cache that never ran initialize() (bare harnesses, pre-init
    // callers). The heavy listeners — metadata compute, related-link sweeps —
    // still wait for initialize() so the startup flood stays cheap; their
    // add/remove calls are idempotent against these.
    this.vault.on("create", (file: TFile) => {
      if (file instanceof TFile) this.addUniqueFile(file);
    });
    this.vault.on("delete", (file: TFile) => {
      if (file instanceof TFile) this.removeUniqueFile(file.name, file);
    });
  ...
  async initialize(): Promise<void> {
    ...
    // Real Obsidian's reconcile is one synchronous pass over the preloaded
    // cache — no per-file await, and only MARKDOWN files enter the link
    // resolver (an attachment has no links to resolve; queueing all 148k
    // files of a code vault made the resolver run for minutes).
    for (const file of this.vault.getAllLoadedFiles()) {
      ...
      const statMatches =
        info !== undefined && file.stat.mtime === info.mtime && file.stat.size === info.size;
      if (file.extension !== "md") {
        // Deliberate deviation from real Obsidian, which re-runs the cheap
        // non-md branch (one fileCache write each) every boot: skip entirely
        // when mtime+size already match the cached entry.
        if (!statMatches) tasks.push(this.computeFileMetadataAsync(file));
        continue;
      }
      if (statMatches && this.metadataCache.has(info.hash)) this.queueFileForLinkResolution(file);
  ...
  private async work(buffer: ArrayBuffer): Promise<CachedMetadata> {
    ...
    const worker = this.getMetadataWorker();
    if (worker) {
      return new Promise<CachedMetadata>((resolve, reject) => {
        this.workerResolve = resolve;
        this.workerReject = reject;
        // Clone, don't transfer: computeMetadataAsync callers keep their buffer.
        worker.postMessage(buffer);
      });
    }
    // No Worker in this environment (jsdom tests): parse inline through the
      ...
      queueMicrotask(() => {
        try {
          this.onReceiveMessageFromWorker({
            data: parseMarkdownMetadata(new TextDecoder().decode(buffer)),
          });
        } catch (error) {
  ...
  /**
   * The real metadata worker (see metadata.worker.ts). Parsing there instead
   * of the main thread is what keeps the UI responsive while a large vault
   * indexes; real Obsidian's cache is built the same way. `undefined` = not
   * yet tried, `null` = unavailable (jsdom) or dead (load/parse crash).
   */
  private getMetadataWorker(): Worker | null {
    if (this.metadataWorker !== undefined) return this.metadataWorker;
    ...
    try {
      ...
      worker.addEventListener("message", (event: MessageEvent<CachedMetadata>) =>
        this.onReceiveMessageFromWorker(event),
      );
      worker.addEventListener("error", (event) => {
        // A dead worker would silently hang the serial queue; fail the file
        // loudly and let the next parse fall back to the inline path.
        this.metadataWorker = null;
        const reject = this.workerReject;
        this.workerResolve = null;
        this.workerReject = null;
        reject?.(new Error(`Metadata worker failed: ${event.message || "worker error"}`));
      });
      this.metadataWorker = worker;
    } catch {
      this.metadataWorker = null;
    }
    return this.metadataWorker;
  }
  ...
  private updateRelatedLinks(names: string[]): void {
    ...
    // Deliberate deviation with identical results: real Obsidian walks ALL
    // cached files here and rejects most via the same hasRelatedLink check
    // (no record → false). Walking the two link records directly visits
    // exactly the sources that can match — markdown notes with links — and
    // skips the other ~145k files of a code vault, per EVERY create/delete/
    // rename event.
    const queued = new Set<string>();
    const queueRelated = (links: Record<string, Record<string, number>>, related: string[]) => {
      for (const source of Object.keys(links)) {
        if (queued.has(source)) continue;
        if (!hasRelatedLink(related, links[source])) continue;
    ...
    queueRelated(this.resolvedLinks, resolvedNames);
    queueRelated(this.unresolvedLinks, unresolvedNames);
  }
  ...
  private getUniqueFiles(name: string): TFile[] {
    // The lookup is maintained incrementally (initialize seed + create /
    // delete / rename handlers), as in real Obsidian. Rebuilding it here made
    // every link-path lookup O(vault) — the dominant post-paint cost on a
    // large vault's link-resolution sweep.
