...
export class MultiAgentView extends ChatView {
  ...
  getDisplayText(): string {
    ...
    return this.agentId && this.agentId !== "default"
      ? STRINGS.room.titleFor(this.agentId)
      : STRINGS.room.title;
  }
  ...
  private syncParticipants(): void {
    ...
    for (const message of this.session.getMessages()) {
      ...
      const entry = authors.get(message.authorId) ?? {
        name: message.authorName ?? message.authorId,
        speaking: false,
      };
      if (!message.closed) entry.speaking = true;
      authors.set(message.authorId, entry);
    }
    const signature = [...authors.entries()]
      .map(([id, entry]) => `${id}:${entry.speaking}`)
      .join(",");
    ...
    createSpan({
      cls: "multi-agent-participants-label",
      text: STRINGS.room.participants,
      parent: this.participantsEl,
    });
    this.chip("you", STRINGS.role.you, false);
    for (const [authorId, entry] of authors) this.chip(authorId, entry.name, entry.speaking);
    if (authors.size === 0) {
      createSpan({
        cls: "multi-agent-participants-hint",
        text: STRINGS.room.participantsHint,
        parent: this.participantsEl,
      });
    }
    this.inviteChip();
  ...
  private chip(id: string, name: string, speaking: boolean): void {
    const chipEl = createSpan({
      cls: `multi-agent-chip${speaking ? " is-speaking" : ""}`,
      parent: this.participantsEl!,
    });
  ...
  private inviteChip(): void {
    const inviteEl = createEl("button", {
      cls: "multi-agent-invite",
      text: "+",
      parent: this.participantsEl!,
    });
