...
function GitReviewPanel({ app, source }: { app: App; source: GitReviewSource }): ReactNode {
  ...
  const load = useCallback(async () => {
    ...
    setFiles(
      source.kind === "working-tree"
        ? await loadWorkingTree(app)
        : await loadCommit(app, source.ref),
    );
  }, [app, source]);
  ...
  const commitSelected = useCallback(
    async (paths: string[], subject: string, body: string) => {
  ...
  if (files === null)
    return <div className="review-empty review-empty-main">Collecting changes…</div>;
...
async function loadWorkingTree(app: App): Promise<ReviewFile[]> {
  ...
  for (const entry of status) {
    ...
    const newContents =
      reviewStatus === "deleted" || !workingFile ? "" : await app.vault.read(workingFile);
    files.push(
      buildReviewFile(
        entry.path,
        reviewStatus,
