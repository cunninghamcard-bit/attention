...
export class PluginManifestValidator {
  validate(manifest: Partial<PluginManifestInput>): ManifestValidationResult {
    ...
    if (!manifest.version)
      issues.push({ field: "version", message: "Plugin version is required.", severity: "error" });
    if (manifest.id && !/^[a-z0-9_-]+$/.test(manifest.id)) {
      issues.push({
        field: "id",
        message: "Plugin id should use lowercase letters, numbers, underscores or dashes.",
        severity: "warning",
      });
    }
