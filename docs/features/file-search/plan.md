...
Group 1 (order 1):
  ...
  Boundary paths:
    ...
    - docs/features/file-search/tasks.md
  Test selectors:
    - SearchView renders Obsidian search structure and grouped matches
    - SearchView persists matching case collapse and sort state
    - SearchView opens a result with the exact match range
    - SearchView handles empty queries and ignores stale results
    - SearchView reports a search error without partial results
