...
test("renders the ![[...]] image embed inside markdown preview", async ({ app }) => {
  ...
  await page.evaluate(() => {
    const app = (window as unknown as { app: { commands: { executeCommandById(id: string): boolean } } }).app;
  ...
  await expect.poll(() => embed.evaluate((el: HTMLImageElement) => el.naturalWidth)).toBeGreaterThan(0);
});
...
async function openFromTree(page: import("@playwright/test").Page, path: string): Promise<void> {
  ...
  if (!(await file.isVisible().catch(() => false))) {
    await page.locator(".nav-folder-title", { hasText: path.split("/")[0] }).first().click();
  }
  await file.click();
}
