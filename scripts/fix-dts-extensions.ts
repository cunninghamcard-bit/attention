...
for (const file of await walk(root)) {
  ...
  const next = source.replace(relativeSpecifier, (match, prefix: string, specifier: string, suffix: string) => {
...
console.log(`Fixed ${changed} declaration import specifier${changed === 1 ? "" : "s"} under ${relative(process.cwd(), root)}`);

await copyFile(join(root, "index.d.ts"), join(root, "index.d.cts"));
console.log(`Wrote CommonJS declaration entry ${relative(process.cwd(), join(root, "index.d.cts"))}`);
...
async function firstExistingDirectory(paths: string[]): Promise<string> {
  ...
  throw new Error(`No declaration output directory found. Tried: ${paths.map((path) => relative(process.cwd(), path)).join(", ")}`);
}
