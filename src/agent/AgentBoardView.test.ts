...
import { AGENT_BOARD_VIEW_TYPE, AgentBoardView } from "./AgentBoardView";
...
describe("AgentBoardView", () => {
  it("renders one card per agent with status and actions", async () => {
    ...
    await app.ready;
    expect(app.viewRegistry.getViewCreatorByType(AGENT_BOARD_VIEW_TYPE)).toBeTypeOf("function");

    const leaf = app.workspace.getLeaf("tab");
    await leaf.setViewState({ type: AGENT_BOARD_VIEW_TYPE, active: true });
    await new Promise((resolve) => setTimeout(resolve, 10));

    const view = leaf.view as AgentBoardView;
  ...
  it("shows the empty state when the bridge has no agents", async () => {
    ...
    await leaf.setViewState({ type: AGENT_BOARD_VIEW_TYPE, active: true });
    await new Promise((resolve) => setTimeout(resolve, 10));
    expect((leaf.view as AgentBoardView).contentEl.querySelector(".agent-board-empty")).not.toBeNull();
    leaf.detach();
  });
});
