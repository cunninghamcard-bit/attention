...
describe("App protocol handlers", () => {
  ...
  it("opens files from obsidian://open with subpath and paneType", async () => {
    const app = new App(document.createElement("div"));
    await app.vault.create("Target.md", "# Heading#Child");

    await app.uriRouter.handleUri("obsidian://open?file=Target%23Heading%23Child%7CAlias&paneType=tab");
  ...
  it("opens global search from obsidian://search with the query", async () => {
    const app = new App(document.createElement("div"));
    await app.corePluginsReady;

    await app.uriRouter.handleUri("obsidian://search?query=needle");
  ...
  it("creates, appends, overwrites, and silently updates files from obsidian://new", async () => {
    const app = new App(document.createElement("div"));

    await app.uriRouter.handleUri(`obsidian://new?${new URLSearchParams({ name: "Protocol", content: "Hello", paneType: "tab" })}`);
    ...
    await app.uriRouter.handleUri(`obsidian://new?${new URLSearchParams({ name: "Protocol", append: "true", content: "World", silent: "true" })}`);

    expect(created ? await app.vault.read(created) : "").toBe("Hello\n\nWorld");

    await app.uriRouter.handleUri(`obsidian://new?${new URLSearchParams({ file: "Protocol.md", overwrite: "true", content: "Replaced", silent: "true" })}`);

    expect(created ? await app.vault.read(created) : "").toBe("Replaced");
  });

  it("reads clipboard content for obsidian://new when clipboard is requested", async () => {
    const app = new App(document.createElement("div"));
    ...
    await app.uriRouter.handleUri("obsidian://new?name=Clipboard&clipboard=true&silent=true");

    const file = app.vault.getFileByPath("Clipboard.md");
    expect(file ? await app.vault.read(file) : "").toBe("Clipboard text");
  });

  it("runs x-success callbacks for obsidian://new when URI callbacks are enabled", async () => {
    const app = new App(document.createElement("div"));
    const open = vi.fn();
    Object.defineProperty(window, "open", { configurable: true, value: open });
    app.vault.setConfig("uriCallbacks", true);

    await app.uriRouter.handleUri(`obsidian://new?${new URLSearchParams({
      name: "Callback",
      content: "Done",
      silent: "true",
    ...
    expect(url.searchParams.get("url")).toBe("obsidian://open?vault=Vault&file=Callback.md");
  });
  ...
  it("copies the active file address or opens x-error from hook-get-address", async () => {
    ...
    expect(writeText).toHaveBeenCalledWith("[Address](obsidian://open?vault=Vault&file=Address.md)");
