  ...
  return {
    ...
    init(_app: App, plugin: InternalPluginWrapper) {
      ...
      plugin.registerViewType(
        GRAPH_VIEW_TYPE,
        (leaf) => new GraphView(leaf, options, { onOptionsChange: saveGraphData }),
      );
    ...
    async onEnable(_app: App, plugin: InternalPluginWrapper) {
      ...
      plugin.registerEvent(
        plugin.app.workspace.on<[Menu, TFile, string, WorkspaceLeaf]>(
          "file-menu",
          (menu, file, source, sourceLeaf) => {
...
function openLinkedLocalGraph(
  app: App,
  path: string,
  sourceLeaf: WorkspaceLeaf | null | undefined,
): void {
  const leaf = app.workspace.splitLeafOrActive(sourceLeaf ?? app.workspace.activeLeaf, "vertical");
  void leaf.setViewState({
    type: LOCAL_GRAPH_VIEW_TYPE,
    active: true,
    group: sourceLeaf ?? app.workspace.activeLeaf,
    state: { file: path },
  });
}
