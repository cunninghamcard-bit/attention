...
export function validateVaultPath(path: string): void {
  ...
  if (path.split("/").some((part) => invalidPattern.test(part))) {
    throw new Error(
      `File name cannot contain any of the following characters: ${formatCharacterList(invalidChars)}`,
    );
  }
}
...
export function validateRenameName(
  ...
  if (requireNonEmpty && name.length === 0)
    return { name, error: "File name cannot be empty", warning: "" };
  ...
  if (name.startsWith("."))
    return { name, error: "File name cannot start with a dot", warning: "" };
  if (vault.checkForDuplicate(file, name))
    return { name, error: "File already exists", warning: "" };
  return {
    name,
    error: "",
    warning: hasUnsafeRenameChars(name)
      ? `File name contains unsafe characters: ${formatCharacterList(unsafeRenameChars)}`
      : "",
  };
}

export function validateRenamePromptName(
  vault: RenameVault,
  file: TAbstractFile,
  rawName: string,
): RenameValidationResult {
  ...
  if (name.startsWith("."))
    return { name, error: "File name cannot start with a dot", warning: "" };
  if (hasUnsafeRenameChars(name))
    return {
      name,
      error: `File name contains unsafe characters: ${formatCharacterList(unsafeRenameChars)}`,
      warning: "",
    };
  if (vault.checkForDuplicate(file, name))
    return { name, error: "File already exists", warning: "" };
...
function validateWindowsSpecialName(pathOrName: string): void {
  ...
  if (/^(CON|PRN|AUX|NUL|COM[1-9]|LPT[1-9])$/i.test(stem))
    throw new Error(`File name is forbidden: ${stem}`);
}
