...
export class CodeFileView extends TextFileView {
  ...
  getIcon(): string {
    return this.file
      ? getFileTypeInfo(this.file.name, this.file.extension).icon
      : "lucide-file-code";
  }
  ...
  override setEphemeralState(state: unknown): void {
    ...
    const { line, matchStart, matchEnd } = state as {
      line?: unknown;
      matchStart?: unknown;
      matchEnd?: unknown;
    };
  ...
  setWordWrap(wordWrap: boolean): void {
    ...
    this.cm?.dispatch({
      effects: this.wrapCompartment.reconfigure(wordWrap ? EditorView.lineWrapping : []),
    });
  }
