...
describe("vaults", () => {
  ...
  it("verbose appends the full path after a tab", async () => {
    ...
    expect(await app.cli.handleCli(["vaults", "verbose"])).toBe("Alpha\t/Users/u/Alpha\nBeta\t/Users/u/Beta");
  });
  ...
  it("throws the desktop-only string on mobile", async () => {
    ...
    await expect(app.cli.handleCli(["vaults"])).rejects.toBe("This command is only available on desktop.");
  });
});
...
describe("folder", () => {
  it("returns the four-line tabbed report", async () => {
    ...
    expect(await app.cli.handleCli(["folder", "path=Folder"])).toBe("path\tFolder\nfiles\t2\nfolders\t1\nsize\t8");
  });
  ...
  it("an unknown info value falls through to the full report", async () => {
    const app = await seededApp();
    expect(await app.cli.handleCli(["folder", "path=Folder", "info=bogus"])).toBe("path\tFolder\nfiles\t2\nfolders\t1\nsize\t8");
  });

  it("throws not-found and file-not-folder strings", async () => {
    const app = await seededApp();
    await expect(app.cli.handleCli(["folder", "path=Nope"])).rejects.toBe('Folder "Nope" not found.');
    await expect(app.cli.handleCli(["folder", "path=Folder/Sub.md"])).rejects.toBe('"Folder/Sub.md" is a file, not a folder.');
  });

  it("missing path fails dispatcher validation, and the handler keeps the reference's own runtime check", async () => {
    const app = await seededApp();
    await expect(app.cli.handleCli(["folder"])).rejects.toMatch(/^Missing required parameter: path/);
...
describe("file", () => {
  ...
  it("path= throws and never falls through to file=", async () => {
    const app = await seededApp();
    await expect(app.cli.handleCli(["file", "path=Nope.md", "file=Note"])).rejects.toBe('File "Nope.md" not found.');
  });

  it("throws folder-not-file, unresolved-name and no-active-file strings", async () => {
    const app = await seededApp();
    await expect(app.cli.handleCli(["file", "path=Folder"])).rejects.toBe('"Folder" is a folder, not a file.');
    await expect(app.cli.handleCli(["file", "file=Nope"])).rejects.toBe('File "Nope" not found.');
    await expect(app.cli.handleCli(["file"])).rejects.toBe("No active file. Use file=<name> or path=<path> to specify a file.");
  });
});
