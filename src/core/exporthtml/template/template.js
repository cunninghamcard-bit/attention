    (function() {
      ...
      function renderBlocks(blocks) {
        ...
        return blocks.map(function (b) {
          ...
          if (b.kind === 'group') {
            var label = b.label ? `<div class="rb-group-label">${escapeHtml(b.label)}</div>` : '';
            return `<div class="rb-group">${label}${renderBlocks(b.children || [])}</div>`;
          }
          if (b.kind === 'text') {
            var cls = b.style ? ` rb-${escapeHtml(b.style)}` : '';
            return `<div class="rb-text${cls}">${escapeHtml(b.text || '')}</div>`;
          }
          if (b.kind === 'badge') {
            var badgeCls = b.style ? ` rb-${escapeHtml(b.style)}` : '';
            return `<div><span class="rb-badge${badgeCls}">${escapeHtml(b.text || '')}</span></div>`;
          }
          if (b.kind === 'code') {
            var codeText = b.text || '';
            if (b.language) {
              try {
                return `<pre class="rb-code"><code class="hljs">${hljs.highlight(codeText, { language: b.language }).value}</code></pre>`;
              } catch (e) { /* unknown language -> plain */ }
            }
            return `<pre class="rb-code"><code>${escapeHtml(codeText)}</code></pre>`;
          }
          if (b.kind === 'diff') {
            var lines = String(b.text || '').split('\n');
            var out = '<div class="tool-diff">';
            for (const line of lines) {
              const cls = line.match(/^\+/) ? 'diff-added' : line.match(/^-/) ? 'diff-removed' : 'diff-context';
              out += `<div class="${cls}">${escapeHtml(replaceTabs(line))}</div>`;
            }
            out += '</div>';
            return out;
          }
          if (b.kind === 'image') {
            var mimeType = escapeHtml(b.mimeType || 'image/png');
            var data = escapeHtml(b.data || '');
            return `<img class="tool-image" src="data:${mimeType};base64,${data}">`;
          }
          return '';
        }).join('');
      }
      ...
      function renderToolCall(call) {
        ...
        const pre = renderedTools?.[call.id];
        if (pre && (pre.callBlocks || pre.resultBlocks)) {
          let html = `<div class="tool-execution ${statusClass}" id="${toolDomId}">`;
          if (pre.callBlocks) {
            html += `<div class="tool-header rb-call">${renderBlocks(pre.callBlocks)}</div>`;
          } else {
            html += `<div class="tool-header"><span class="tool-name">${escapeHtml(name)}</span></div>`;
          }
          if (pre.resultBlocks && pre.resultBlocksExpanded && pre.resultBlocksExpanded.length) {
            html += `<div class="tool-output expandable rb-output" onclick="if(window.getSelection().toString())return;this.classList.toggle('expanded')">
        <div class="output-preview">${renderBlocks(pre.resultBlocks)}</div>
        <div class="output-full">${renderBlocks(pre.resultBlocksExpanded)}</div>
      </div>`;
          } else if (pre.resultBlocks) {
            html += `<div class="tool-output rb-output">${renderBlocks(pre.resultBlocks)}</div>`;
          }
          html += '</div>';
          return html;
        }
