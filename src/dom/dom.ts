...
export type DomElementSpec = ClassValue | DomElementInfo | ((el: HTMLElement) => unknown) | undefined;
export type DomElementCallback<T extends HTMLElement> = (el: T) => unknown;

export function createDiv(spec?: DomElementSpec, parentOrCallback?: DomParent | DomElementCallback<HTMLDivElement>, callback?: DomElementCallback<HTMLDivElement>): HTMLDivElement {
  return createEl("div", spec, parentOrCallback, callback);
}

export function createSpan(spec?: DomElementSpec, parentOrCallback?: DomParent | DomElementCallback<HTMLSpanElement>, callback?: DomElementCallback<HTMLSpanElement>): HTMLSpanElement {
...
export function installDomExtensions(win: Window & typeof globalThis = window): void {
  ...
  defineMethod(nodeProto, "createEl", function createElMethod<K extends keyof HTMLElementTagNameMap>(this: ParentNode & Node, tag: K, spec?: DomElementSpec, callback?: DomElementCallback<HTMLElementTagNameMap[K]>): HTMLElementTagNameMap[K] {
    return createEl(tag, spec, this, callback);
  });
  defineMethod(nodeProto, "createDiv", function createDivMethod(this: ParentNode & Node, spec?: DomElementSpec, callback?: DomElementCallback<HTMLDivElement>): HTMLDivElement {
    return createEl("div", spec, this, callback);
  });
  defineMethod(nodeProto, "createSpan", function createSpanMethod(this: ParentNode & Node, spec?: DomElementSpec, callback?: DomElementCallback<HTMLSpanElement>): HTMLSpanElement {
  ...
  defineMethod(nodeProto, "setChildrenInPlace", function setChildrenInPlaceMethod(this: Node, children: Node[]): Node {
  ...
  defineMethod(elementProto, "setText", function setTextMethod(this: Element, text: string): Element {
    this.textContent = text;
    return this;
  });
  defineMethod(elementProto, "addClass", function addClassMethod(this: Element, cls: ClassValue): Element {
    addClass(this, cls);
    return this;
  });
  defineMethod(elementProto, "addClasses", function addClassesMethod(this: Element, classes: string[]): Element {
    addClass(this, classes);
    return this;
  });
  defineMethod(elementProto, "removeClass", function removeClassMethod(this: Element, cls: ClassValue): Element {
    removeClass(this, cls);
    return this;
  });
  defineMethod(elementProto, "removeClasses", function removeClassesMethod(this: Element, classes: string[]): Element {
    removeClass(this, classes);
    return this;
  });
  defineMethod(elementProto, "toggleClass", function toggleClassMethod(this: Element, cls: string, value?: boolean): Element {
    this.classList.toggle(cls, value);
    return this;
  });
  defineMethod(elementProto, "hasClass", function hasClassMethod(this: Element, cls: string): boolean {
    return this.classList.contains(cls);
  });
  defineMethod(elementProto, "setAttr", function setAttrMethod(this: Element, name: string, value: AttributeValue): Element {
    setAttributeValue(this, name, value);
    return this;
  });
  defineMethod(elementProto, "getAttr", function getAttrMethod(this: Element, name: string): string {
    return this.getAttribute(name) ?? "";
  });
  defineMethod(elementProto, "toggle", function toggleMethod(this: HTMLElement, show?: boolean): void {
  ...
  defineMethod(elementProto, "onNodeInserted", function onNodeInsertedMethod(this: Element, listener: () => unknown, once = false): () => void {
    const target = this;
  ...
  defineMethod(elementProto, "find", function findMethod(this: Element, selector: string): Element | null {
    return this.querySelector(selector);
  });
  defineMethod(elementProto, "findAll", function findAllMethod(this: Element, selector: string): NodeListOf<Element> {
    return this.querySelectorAll(selector);
  });
  defineMethod(elementProto, "on", function onMethod(this: Element, type: string, selector: string, listener: (event: Event, target: HTMLElement) => unknown): () => void {
  ...
  defineMethod(elementProto, "onClickEvent", function onClickEventMethod(this: Element, listener: (event: MouseEvent) => unknown): () => void {
    const handler = (event: Event) => listener(event as MouseEvent);
    this.addEventListener("click", handler);
    return () => this.removeEventListener("click", handler);
  });
  defineMethod(elementProto, "matchParent", function matchParentMethod(this: Element, selector: string): Element | null {
...
declare global {
  interface Node {
    ...
    readonly win: Window;
    createEl<K extends keyof HTMLElementTagNameMap>(tag: K, spec?: DomElementSpec, callback?: DomElementCallback<HTMLElementTagNameMap[K]>): HTMLElementTagNameMap[K];
    createDiv(spec?: DomElementSpec, callback?: DomElementCallback<HTMLDivElement>): HTMLDivElement;
    createSpan(spec?: DomElementSpec, callback?: DomElementCallback<HTMLSpanElement>): HTMLSpanElement;
  ...
  interface Element {
    ...
    on(type: string, selector: string, listener: (event: Event, target: HTMLElement) => unknown): () => void;
...
function normalizeCreateArgs<T extends HTMLElement>(
  ...
  let normalizedSpec: ClassValue | DomElementInfo | undefined = typeof spec === "function" ? undefined : spec;
  let normalizedParent: DomParent | null = isDomParent(parentOrCallback) ? parentOrCallback : null;
  let normalizedCallback: DomElementCallback<T> | null = typeof spec === "function" ? spec as DomElementCallback<T> : null;
  if (typeof parentOrCallback === "function") normalizedCallback = parentOrCallback;
  if (callback) normalizedCallback = callback;
  if (normalizedSpec && typeof normalizedSpec === "object" && !Array.isArray(normalizedSpec) && "parent" in normalizedSpec) {
...
function isDomParent(value: unknown): value is DomParent {
  return typeof value === "object" && value !== null && typeof (value as { appendChild?: unknown }).appendChild === "function";
}
