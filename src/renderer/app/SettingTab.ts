...
} from "../ui/suggest/FileInputSuggest";
import { Setting, SettingGroup, type ExtraButtonComponent, type SettingText } from "../ui/Setting";
...
export interface SettingToggleControl<K extends string = string> extends SettingControlBase<
  boolean,
  K
> {
  type: "toggle";
...
export interface SettingDropdownControl<K extends string = string> extends SettingControlBase<
  string,
  K
> {
  type: "dropdown";
...
export interface SettingTextControl<K extends string = string> extends SettingControlBase<
  string,
  K
> {
  type: "text";
...
export interface SettingTextAreaControl<K extends string = string> extends SettingControlBase<
  string,
  K
> {
  type: "textarea";
...
export interface SettingNumberControl<K extends string = string> extends SettingControlBase<
  number,
  K
> {
  type: "number";
...
export interface SettingFileControl<K extends string = string> extends SettingControlBase<
  string,
  K
> {
  type: "file";
...
export interface SettingFolderControl<K extends string = string> extends SettingControlBase<
  string,
  K
> {
  type: "folder";
...
export interface SettingSliderControl<K extends string = string> extends SettingControlBase<
  number,
  K
> {
  type: "slider";
...
export interface SettingColorControl<K extends string = string> extends SettingControlBase<
  HexString,
  K
> {
  type: "color";
...
export type SettingGroupItem<K extends string = string> =
  | SettingDefinition<K>
...
function getStatus(
  value: "warning" | null | (() => "warning" | null) | undefined,
): "warning" | null {
...
): item is SettingDefinitionGroup | SettingDefinitionList {
  return "type" in item && (item.type === "group" || item.type === "list");
}

function isListDefinition(
  item: SettingDefinitionGroup | SettingDefinitionList,
): item is SettingDefinitionList {
  return item.type === "list";
}

function isPageDefinition(
  item: SettingDefinitionItem | SettingGroupItem,
): item is SettingDefinitionPage {
...
export const SettingTab: { new (app: App, setting: SettingRegistry): SettingTabRuntime } =
  class SettingTab {
    app: App;
    setting: SettingRegistry;
    containerEl: HTMLElement;
    ...
    refreshDomState(): void {
      for (const state of this.renderedDomStates) {
        if (state.visible || state.search)
          state.setVisible?.((state.visible?.() ?? true) && (state.search?.() ?? true));
    ...
    private renderGroup(
      definition: SettingDefinitionGroup | SettingDefinitionList,
      containerEl: HTMLElement,
      path: string,
    ): SettingGroup {
      ...
      if (listDefinition?.addItem) {
        ...
        group.addExtraButton((button) => {
          button
            ...
            .onClick(() => addItem.action(button.extraSettingsEl));
        });
      }
      ...
      if (listDefinition && items.length === 0 && listDefinition.emptyState) {
        ...
        items.forEach((item, index) =>
          this.renderGroupItem(item, group, index, listDefinition, `${path}/${index}`, groupSearch),
        );
      }
    ...
    private renderPage(
      definition: SettingDefinitionPage,
      group: SettingGroup,
      index: number,
      list?: SettingDefinitionList,
      path = `page/${index}`,
    ...
    private matchesSettingSearch(
      definition: SettingDefinition,
      groupSearch?: GroupSearchFilter,
    ): boolean {
      return (
        this.matchesGlobalSettingSearch(definition) &&
        this.matchesGroupSettingSearch(definition, groupSearch)
      );
    }

    private matchesPageSearch(
      definition: SettingDefinitionPage,
      groupSearch?: GroupSearchFilter,
    ): boolean {
      return (
        this.matchesGlobalPageSearch(definition) &&
        this.matchesGroupPageSearch(definition, groupSearch)
      );
    }
    ...
    private matchesGroupSettingSearch(
      definition: SettingDefinition,
      groupSearch?: GroupSearchFilter,
    ): boolean {
    ...
    private matchesGroupPageSearch(
      _definition: SettingDefinitionPage,
      groupSearch?: GroupSearchFilter,
    ): boolean {
      if (!groupSearch) return true;
      const query = this.groupSearchQueries.get(groupSearch.path) ?? "";
    ...
    private getDefinitionSearchText(definition: SettingDefinition): string {
      return [definition.name, getSearchText(definition.desc), ...(definition.aliases ?? [])]
        .join(" ")
        .toLowerCase();
    }
        ...
        case "number": {
          ...
          setting.addText((component) => {
            ...
            component.onChange((next) => {
              const numberValue = this.coerceNumber(
    ...
    private validateInitialValue<V>(
      setting: Setting,
      control: SettingControlBase<V>,
      value: V,
    ): void {
      this.validateControl(setting, control, value, false);
    }

    private handleControlChange<V>(
      setting: Setting,
      control: SettingControlBase<V>,
      value: V,
    ): void {
