...
} from "../metadata/LinkSuggestionManager";
...
export class LinkSuggest extends EditorSuggest<LinkFileSuggestion> {
  ...
  async getSuggestions(context: EditorSuggestContext): Promise<LinkFileSuggestion[]> {
    ...
    return this.app.linkSuggestions.getSuggestionsAsync(
      null,
      linkContext.query,
  ...
  private async acceptSuggestion(
    value: LinkFileSuggestion,
    context: LinkSuggestContext,
    ...
    context.editor.replaceRange(
      ...
      { line: context.start.line, ch: replacement.start },
      { line: context.end.line, ch: replacement.end },
    );
  }

  private async applyToMarkdownView(
    view: MarkdownView,
    value: LinkFileSuggestion,
    context: LinkSuggestContext,
    replacement: LinkSuggestionReplacement,
  ): Promise<void> {
    ...
    if (value.type === "block" && replacement.blockId) {
      if (value.file.path === context.sourcePath) {
        ...
        const insertAt = findCurrentBlockInsertionOffset(
          source,
          value.content,
        ...
        if (insertAt <= lineOffset + replacement.selectionStart)
          selectionShift += insertion.addition.length;
...
function suggestionNote(value: LinkFileSuggestion): string {
  ...
  if (value.type === "heading")
    return `${formatPath(value.file, value.path ?? "")}${value.subpath}`;
...
function findCurrentBlockInsertionOffset(
  source: string,
