...
import { EditorSuggestManager } from "../../ui/suggest/EditorSuggest";
import { TFile, type TAbstractFile } from "../../vault/TAbstractFile";
import { DynamicScope } from "../../app/hotkeys/Scope";
...
function createDebouncedWorkspaceRequest<T>(
  ...
  request.run = () => {
    if (runOnlyWhenPending && timer === undefined) return;
    cancel();
    if (!canRun()) return;
    return save();
  };
