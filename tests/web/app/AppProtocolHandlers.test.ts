...
describe("App protocol handlers", () => {
  ...
  it("opens files from attention://open with subpath and paneType", async () => {
    const app = new App(document.createElement("div"));
    await app.vault.create("Target.md", "# Heading#Child");

    await app.uriRouter.handleUri(
      "attention://open?file=Target%23Heading%23Child%7CAlias&paneType=tab",
    );
  ...
  it("opens global search from attention://search with the query", async () => {
    const app = new App(document.createElement("div"));
    await app.corePluginsReady;

    await app.uriRouter.handleUri("attention://search?query=needle");
  ...
  it("creates, appends, overwrites, and silently updates files from attention://new", async () => {
    const app = new App(document.createElement("div"));

    await app.uriRouter.handleUri(
      `attention://new?${new URLSearchParams({ name: "Protocol", content: "Hello", paneType: "tab" })}`,
    );
    ...
    await app.uriRouter.handleUri(
      `attention://new?${new URLSearchParams({ name: "Protocol", append: "true", content: "World", silent: "true" })}`,
    );

    expect(created ? await app.vault.read(created) : "").toBe("Hello\n\nWorld");

    await app.uriRouter.handleUri(
      `attention://new?${new URLSearchParams({ file: "Protocol.md", overwrite: "true", content: "Replaced", silent: "true" })}`,
    );

    expect(created ? await app.vault.read(created) : "").toBe("Replaced");
  });

  it("reads clipboard content for attention://new when clipboard is requested", async () => {
    const app = new App(document.createElement("div"));
    ...
    await app.uriRouter.handleUri("attention://new?name=Clipboard&clipboard=true&silent=true");

    const file = app.vault.getFileByPath("Clipboard.md");
    expect(file ? await app.vault.read(file) : "").toBe("Clipboard text");
  });

  it("runs x-success callbacks for attention://new when URI callbacks are enabled", async () => {
    const app = new App(document.createElement("div"));
    const open = vi.fn();
    ...
    await app.uriRouter.handleUri(
      `attention://new?${new URLSearchParams({
        name: "Callback",
        content: "Done",
        silent: "true",
    ...
    expect(url.searchParams.get("url")).toBe("attention://open?vault=In-memory&file=Callback");
  });
  ...
  it("copies the active file address or opens x-error from hook-get-address", async () => {
    ...
    await app.uriRouter.handleUri("attention://hook-get-address");

    expect(writeText).toHaveBeenCalledWith(
      "[Address](attention://open?vault=In-memory&file=Address)",
    );

    await app.workspace.activeLeaf?.setViewState({ type: "empty", active: true });
    await app.uriRouter.handleUri(
      `attention://hook-get-address?${new URLSearchParams({ "x-error": "callback://error" })}`,
    );
