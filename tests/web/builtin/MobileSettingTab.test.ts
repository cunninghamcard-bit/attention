...
describe("MobileSettingTab Obsidian mobile toolbar settings", () => {
  ...
  it("renders selected mobile toolbar commands with Obsidian option rows", () => {
    ...
    const rows = [
      ...tab.containerEl.querySelectorAll<HTMLElement>(
        ".mobile-toolbar-selected-list .mobile-option-setting-item",
      ),
    ];
    expect(
      rows.map((row) => row.querySelector(".mobile-option-setting-item-name")?.textContent),
    ).toEqual(["Insert Markdown link", "Toggle bold"]);
  ...
  it("removes, moves, drags, and adds toolbar commands through vault config", () => {
    ...
    const moreRows = [
      ...tab.containerEl.querySelectorAll<HTMLElement>(
        ".mobile-toolbar-more-list .mobile-option-setting-item",
      ),
    ];
    ...
    tab.display();
    const search = tab.containerEl.querySelector<HTMLInputElement>(
      ".mobile-toolbar-command-search",
    );
    if (!search) throw new Error("Missing search");
    search.value = "Gamma";
    search.dispatchEvent(new Event("input", { bubbles: true }));
    const result = tab.containerEl.querySelector<HTMLElement>(
      ".mobile-toolbar-command-results .suggestion-item",
    );
  ...
  it("opens the mobile settings tab from the mobile-only configure-toolbar command", async () => {
    ...
    try {
      ...
      expect(
        document.body.querySelector('.vertical-tab-nav-item.is-active[data-setting-id="mobile"]'),
      ).not.toBeNull();
