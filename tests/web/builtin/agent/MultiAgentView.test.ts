...
import { MULTI_AGENT_VIEW_TYPE, MultiAgentView } from "@web/builtin/agent/MultiAgentView";
...
describe("MultiAgentView", () => {
  it("renders authored messages and derives the participants strip", async () => {
    ...
    expect(
      el.querySelector('.chat-message[data-author-id="researcher"]') as HTMLElement | null,
    ).not.toBeNull();

    const chips = [...el.querySelectorAll(".multi-agent-chip .multi-agent-chip-name")].map(
      (n) => n.textContent,
    );
    ...
    expect((view as unknown as { mentionTargets(): string[] }).mentionTargets().sort()).toEqual([
      "工程师",
      "研究员",
    ]);
  });
