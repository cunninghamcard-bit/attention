...
import { formatDate } from "@web/builtin/DailyNotes";
import type { TemplatesController } from "@web/builtin/Templates";
import { TFile } from "@web/vault/TAbstractFile";
...
describe("create", () => {
  ...
  it("joins path= and name=, stripping trailing slashes from path", async () => {
    const app = await seededApp();
    expect(await app.cli.handleCli(["create", "path=Folder/", "name=Deep"])).toBe(
      "Created: Folder/Deep.md",
    );
  });

  it("rejects a name containing a slash", async () => {
    const app = await seededApp();
    await expect(app.cli.handleCli(["create", "name=a/b"])).rejects.toBe(
      'name cannot contain "/". Use path for a full file path.',
    );
  });
  ...
  it("creates missing intermediate folders", async () => {
    const app = await seededApp();
    expect(await app.cli.handleCli(["create", "path=New/Sub/Leaf"])).toBe(
      "Created: New/Sub/Leaf.md",
    );
  });

  it("overwrite modifies an existing file in place", async () => {
    const app = await seededApp();
    expect(await app.cli.handleCli(["create", "name=Note", "overwrite", "content=new"])).toBe(
      "Overwrote: Note.md",
    );
  ...
  it("quirk: overwrite over a folder creates a fresh file but says Overwrote", async () => {
    ...
    expect(await app.cli.handleCli(["create", "name=Weird", "overwrite"])).toBe(
      "Overwrote: Weird 1.md",
    );
  });
...
describe("create with template", () => {
  it("throws when the templates plugin is not enabled", async () => {
    const app = await seededApp();
    await expect(app.cli.handleCli(["create", "name=T", "template=X"])).rejects.toBe(
      "Templates plugin is not enabled.",
    );
  });
  ...
  it("renders {{title}} and {{date}}/{{date:FMT}} from the template; content= is ignored", async () => {
    ...
    expect(await app.cli.handleCli(["create", "name=T", "template=Tpl", "content=ignored"])).toBe(
      "Created: T.md",
    );
    const now = new Date();
  ...
  it("matches templates case-insensitively by folder-relative path", async () => {
    ...
    expect(await app.cli.handleCli(["create", "name=D", "template=sub/deep"])).toBe(
      "Created: D.md",
    );
  ...
  it("reports an unknown template", async () => {
    ...
    await enableTemplates(app);
    await app.vault.create("Templates/Tpl.md", "x");
    await expect(app.cli.handleCli(["create", "name=T", "template=Nope"])).rejects.toBe(
  ...
  it("reports a missing template folder", async () => {
    const app = await seededApp();
    await enableTemplates(app);
    await expect(app.cli.handleCli(["create", "name=T", "template=Tpl"])).rejects.toBe(
      'Template folder "Templates" not found.',
    );
  });
...
describe("append", () => {
  it("appends with a leading newline by default", async () => {
    const app = await seededApp();
    expect(await app.cli.handleCli(["append", "path=Note.md", "content=more"])).toBe(
      "Appended to: Note.md",
    );
    expect(await readPath(app, "Note.md")).toBe("# Note\nbody\nmore");
  });
  ...
  it("empty content= throws the handler usage error", async () => {
    const app = await seededApp();
    await expect(app.cli.handleCli(["append", "path=Note.md", "content="])).rejects.toBe(
  ...
  it("absent content is rejected by the dispatcher's generic required check", async () => {
    const app = await seededApp();
    await expect(app.cli.handleCli(["append", "path=Note.md"])).rejects.toBe(
      "Missing required parameter: content=<text>\nUsage: append [file=<name>] [path=<path>] content=<text> [inline]",
    );
  });

  it("resolves file= like a wikilink", async () => {
    const app = await seededApp();
    expect(await app.cli.handleCli(["append", "file=Sub", "content=x"])).toBe(
      "Appended to: Folder/Sub.md",
    );
  });

  it("throws each tryResolveFile error string", async () => {
    const app = await seededApp();
    await expect(app.cli.handleCli(["append", "path=Nope.md", "content=x"])).rejects.toBe(
      'File "Nope.md" not found.',
    );
    await expect(app.cli.handleCli(["append", "path=Folder", "content=x"])).rejects.toBe(
      '"Folder" is a folder, not a file.',
    );
    await expect(app.cli.handleCli(["append", "file=Ghost", "content=x"])).rejects.toBe(
      'File "Ghost" not found.',
    );
    // Resolution runs before the empty-content check.
    await expect(app.cli.handleCli(["append", "content="])).rejects.toBe(
...
describe("prepend", () => {
  it("inserts at the very top when there is no frontmatter", async () => {
    const app = await seededApp();
    expect(await app.cli.handleCli(["prepend", "path=Note.md", "content=top"])).toBe(
  ...
  it("empty content= throws the handler usage error", async () => {
    const app = await seededApp();
    await expect(app.cli.handleCli(["prepend", "path=Note.md", "content="])).rejects.toBe(
...
describe("move", () => {
  it("moves into a folder keeping the full filename", async () => {
    const app = await seededApp();
    expect(await app.cli.handleCli(["move", "path=Note.md", "to=Folder"])).toBe(
      "Moved: Note.md -> Folder/Note.md",
    );
  ...
  it("treats a destination containing a dot as a full file path", async () => {
    const app = await seededApp();
    expect(await app.cli.handleCli(["move", "path=Note.md", "to=Folder/New.md"])).toBe(
      "Moved: Note.md -> Folder/New.md",
    );
  });

  it("to=/ moves to the vault root", async () => {
    const app = await seededApp();
    expect(await app.cli.handleCli(["move", "path=Folder/Sub.md", "to=/"])).toBe(
      "Moved: Folder/Sub.md -> Sub.md",
    );
  });

  it("empty to= throws the handler usage error (distinct from the dispatcher's)", async () => {
    const app = await seededApp();
    await expect(app.cli.handleCli(["move", "path=Note.md", "to="])).rejects.toBe(
      "Missing required parameter: to\nUsage: move to=<folder> or to=<path>",
    );
    await expect(app.cli.handleCli(["move", "path=Note.md"])).rejects.toBe(
      "Missing required parameter: to=<path>\nUsage: move [file=<name>] [path=<path>] to=<path>",
    );
  });
  ...
  it("a taken destination rejects from the vault, not this handler", async () => {
    ...
    await expect(app.cli.handleCli(["move", "path=Other.md", "to=Note.md"])).rejects.toThrow(
      "File already exists",
    );
  });
  ...
  it("throws the resolve error for a missing source", async () => {
    const app = await seededApp();
    await expect(app.cli.handleCli(["move", "path=Nope.md", "to=Folder"])).rejects.toBe(
      'File "Nope.md" not found.',
    );
  });
});
...
describe("rename", () => {
  it("preserves the extension for a bare name", async () => {
    const app = await seededApp();
    expect(await app.cli.handleCli(["rename", "path=Note.md", "name=Renamed"])).toBe(
      "Renamed: Note.md -> Renamed.md",
    );
  ...
  it("takes a dotted name literally", async () => {
    const app = await seededApp();
    expect(await app.cli.handleCli(["rename", "path=Note.md", "name=raw.txt"])).toBe(
      "Renamed: Note.md -> raw.txt",
    );
  });

  it("stays in the same folder", async () => {
    const app = await seededApp();
    expect(await app.cli.handleCli(["rename", "path=Folder/Sub.md", "name=New"])).toBe(
      "Renamed: Folder/Sub.md -> Folder/New.md",
    );
  });

  it("empty name= throws the handler usage error (distinct from the dispatcher's)", async () => {
    const app = await seededApp();
    await expect(app.cli.handleCli(["rename", "path=Note.md", "name="])).rejects.toBe(
      "Missing required parameter: name\nUsage: rename name=<new name>",
    );
    await expect(app.cli.handleCli(["rename", "path=Note.md"])).rejects.toBe(
  ...
  it("throws the resolve error for a missing source", async () => {
    const app = await seededApp();
    await expect(app.cli.handleCli(["rename", "path=Nope.md", "name=X"])).rejects.toBe(
      'File "Nope.md" not found.',
    );
  });
});
...
describe("delete", () => {
  it("permanent removes the file outright", async () => {
    const app = await seededApp();
    expect(await app.cli.handleCli(["delete", "path=Note.md", "permanent"])).toBe(
      "Deleted permanently: Note.md",
    );
    expect(app.vault.getAbstractFileByPath("Note.md")).toBeNull();
  });
  ...
  it("throws without an active file or a resolvable target", async () => {
    const app = await seededApp();
    await expect(app.cli.handleCli(["delete"])).rejects.toBe(
      "No active file. Use file=<name> or path=<path> to specify a file.",
    );
    await expect(app.cli.handleCli(["delete", "path=Nope.md"])).rejects.toBe(
      'File "Nope.md" not found.',
    );
  });
});
...
describe("move into a missing folder", () => {
  it("propagates the vault's ENOENT (adapter parity) instead of reparenting", async () => {
    const app = await seededApp();
    await expect(app.cli.handleCli(["move", "file=Note", "to=Nope"])).rejects.toThrow(
