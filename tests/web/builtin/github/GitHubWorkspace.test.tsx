...
import {
  ...
  openCommitDetail,
  openGitHubWorkspace,
} from "@web/builtin/github/GitHubWorkspace";
...
function fakeGit(): ElectronGitApi {
  return {
    ...
    async exec(args: string[]): Promise<GitExecResult> {
      ...
      if (args[0] === "remote")
        return { code: 0, stdout: "https://github.com/coder/ghostty-web.git\n", stderr: "" };
  ...
  app.github.transportFactory =
    ...
    async ({ url, method, headers }) => {
      ...
      if (verb === "GET" && path === "/user")
        return json({ login: "cunninghamcard-bit", avatar_url: "", name: "Card" });
      if (verb === "GET" && path === "/repos/coder/ghostty-web")
      ...
      if (
        verb === "GET" &&
        path.startsWith("/repos/coder/ghostty-web/commits?") &&
      ...
      if (verb === "GET" && path.startsWith("/repos/coder/ghostty-web/commits/deadbeef")) {
        if (headers?.Accept?.includes("diff"))
          return { status: 200, text: "diff --git a/x b/x\n", json: null };
        return json({
          sha: "deadbeef01234567",
...
describe("GitHub workspace commits", () => {
  it("opens commits section and lists commits", async () => {
    ...
    const view = app.workspace.getLeavesOfType(GitHubWorkspaceView.VIEW_TYPE)[0]
      .view as GitHubWorkspaceView;
    await until(
      () => view.contentEl.textContent?.includes("fix: powerline") === true,
      "commit row",
    );
