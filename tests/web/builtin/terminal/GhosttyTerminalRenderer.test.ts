...
import {
  ...
  buildTerminalTheme,
  DEFAULT_FONT_STACK,
} from "@web/builtin/terminal/GhosttyTerminalRenderer";
...
describe("buildTerminalTheme", () => {
  it("uses exact bundled font families and real normal/bold Nerd faces", () => {
    ...
    expect(
      BUNDLED_FONT_FACES.filter((face) => face.family === "Workbench Nerd Symbols").map(
        (face) => face.weight,
      ),
    ).toEqual(["400", "700"]);
  });
  ...
  it("always returns a complete theme — ghostty's buildWasmConfig turns missing fields black", () => {
    ...
    for (const dark of [false, true]) {
      ...
      for (const key of required) {
        expect(theme[key], `${dark ? "dark" : "light"} theme missing ${key}`).toMatch(
          /^#[0-9a-f]{6}$/,
        );
      }
    }
