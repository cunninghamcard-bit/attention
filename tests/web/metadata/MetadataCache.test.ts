...
import { MemoryMetadataCacheStore } from "@web/metadata/MetadataCacheStore";
import { Vault, type VaultAdapter } from "@web/vault/Vault";
import { ViewRegistry } from "@web/views/workspace/ViewRegistry";
import { installSlowIndexingNotice } from "@web/app/MetadataIndexingNotice";
...
describe("MetadataCache", () => {
  ...
  it("prunes preloaded entries for files absent from the vault on initialize", async () => {
    ...
    const ghost = await first.create("Plugin Architecture.md", "body #attention tag");
