...
import { View, type ViewState } from "@web/views/View";
import { MarkdownView } from "@web/views/MarkdownView";
import { DeferredView } from "@web/views/DeferredView";
import { Menu } from "@web/ui/Menu";
import { Tasks } from "@web/app/QuitEvent";
import { WorkspaceWindow } from "@web/views/workspace/WorkspaceWindow";
import type { OpenViewState } from "@web/views/workspace/Workspace";
...
describe("Workspace public API parity", () => {
  ...
  it("keeps workspace protocol handler registration atomic and scoped", async () => {
    ...
    await expect(app.uriRouter.handleUri("attention://plugin-action?source=first")).resolves.toBe(
      true,
    );
    ...
    await expect(
      app.uriRouter.handleUri("attention://plugin-action?source=still-first"),
    ).resolves.toBe(true);
    ...
    await expect(app.uriRouter.handleUri("attention://plugin-action")).resolves.toBe(false);
  });
