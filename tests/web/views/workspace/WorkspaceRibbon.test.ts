...
describe("WorkspaceRibbon Obsidian parity", () => {
  ...
  it("reorders ribbon actions through native drag and drop", () => {
    ...
    Object.defineProperty(app.workspace, "requestSaveLayout", {
      configurable: true,
      value: saveLayout,
    });
    ...
    expect(
      Array.from(ribbon.actionsEl?.children ?? []).map((el) => el.getAttribute("aria-label")),
    ).toEqual(["Beta", "Gamma", "Alpha"]);
    expect(Object.keys(ribbon.serialize().hiddenItems as Record<string, boolean>)).toEqual([
      "beta",
      "gamma",
      "alpha",
    ]);
